...
// 这个逻辑 应该是这样的
func (g *GengineParserListener) EnterAggStmt(ctx *parser.AggStmtContext) {
	if len(g.ParseErrors) > 0 {
		return
	}
	agStmt := &base.AggStatement{}
	g.Stack.Push(agStmt)
}

//
func (g *GengineParserListener) ExitAggStmt(ctx *parser.AggStmtContext) {
	if len(g.ParseErrors) > 0 {
		return
	}

	aggStmt := g.Stack.Pop().(*base.AggStatement)
	aggStmt.Sss = ctx.GetText()

	statement := g.Stack.Peek().(*base.Statement)
	statement.AggStatement = aggStmt
}

// todo listener 模式太繁琐了
func (g *GengineParserListener) EnterCountOperator(ctx *parser.CountOperatorContext) {
	if len(g.ParseErrors) > 0 {
		return
	}
	ccb := ctx.GetText()
...
func (g *GengineParserListener) ExitCountOperator(ctx *parser.CountOperatorContext) {
	if len(g.ParseErrors) > 0 {
		return
	}
	cStr := g.Stack.Pop().(string)
	statement := g.Stack.Peek().(*base.AggStatement)
	statement.Countopert = cStr
}

func (g *GengineParserListener) EnterTimeForm(c *parser.TimeFormContext) {

}

// EnterExpireTime is called when entering the expireTime production.
func (g *GengineParserListener) EnterExpireTime(c *parser.ExpireTimeContext) {

}

// EnterExtreValue is called when entering the extreValue production.
func (g *GengineParserListener) EnterExtreValue(c *parser.ExtreValueContext) {

}

func (g *GengineParserListener) ExitTimeForm(c *parser.TimeFormContext) {

}

// EnterExpireTime is called when entering the expireTime production.
func (g *GengineParserListener) ExitExpireTime(c *parser.ExpireTimeContext) {
	bcc := c.GetText()
	staement := g.Stack.Peek().(*base.AggStatement)
	staement.ExpireTime, _ = strconv.Atoi(bcc)
}

// EnterExtreValue is called when entering the extreValue production.
func (g *GengineParserListener) ExitExtreValue(c *parser.ExtreValueContext) {
	bcc := c.GetText()
	parent := c.GetParent()
	fmt.Print(parent)
	staement := g.Stack.Peek().(*base.AggStatement)
	staement.ExtreValue, _ = strconv.ParseInt(bcc, 10, 64)
}
